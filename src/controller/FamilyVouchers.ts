...
// // src/controller/FamilyVouchers.ts
// import { Prisma, PrismaClient } from "@prisma/client";
// import { Request, Response } from "express";
...
//   const start = new Date(`${year}-01-01T00:00:00.000Z`);
//   const end = new Date(`${year + 1}-01-01T00:00:00.000Z`);
//   const count = await (tx as any).familyPaymentVoucher.count({
//     where: { createdAt: { gte: start, lt: end } },
//   });
//   return `VCH-${year}-${String(count + 1).padStart(6, "00")}`;
// }
...
//     return res
//       .status(400)
//       .json({ message: "parentUserId and lines[] are required" });
//   }

//   try {
//     const voucher = await prisma.$transaction(async (tx: Tx) => {
//       if (idempotencyKey) {
//         const existed = await (tx as any).familyPaymentVoucher.findFirst({
//           where: { notes: `IDEMP:${idempotencyKey}` },
//           include: {
//             parentUser: true,
//             createdBy: true,
...
//         select: { id: true, parentUserId: true },
//       });
//       if (kids.length !== studentIds.length)
...
//       const voucherNo = await generateVoucherNo(tx);
//       const baseNotes = idempotencyKey
//         ? `IDEMP:${idempotencyKey}`
//         : notes ?? null;

//       const createdVoucher = await (tx as any).familyPaymentVoucher.create({
//         data: {
//           voucherNo,
//           parentUserId: Number(parentUserId),
//           method,
...
//             discount: disc,
//             Description: notes || "Family voucher",
//             // FIX: field name must match schema
//             familyPaymentVoucherId: createdVoucher.id,
//           },
//         });
...
//             id: true,
//             student_fee: true,
//             // FIX: collection name is lowerCamelCase in Prisma Client
...
//           const base = D(fee.student_fee || 0);
//           const already = Dsum(fee.paymentAllocations.map((a) => a.amount));
//           const due = base.minus(already);
//           if (due.lte(0)) continue;
...
//         if (remaining.gt(0)) {
//           const acct = await tx.studentAccount.findUnique({
//             where: { studentId: Number(line.studentId) },
//           });
//           if (!acct) {
//             await tx.studentAccount.create({
...
//       const enriched = await (tx as any).familyPaymentVoucher.findUnique({
//         where: { id: createdVoucher.id },
//         include: {
//           parentUser: {
//             select: { id: true, fullName: true, phoneNumber: true },
...
//     return res
//       .status(500)
//       .json({ message: err?.message || "Failed to create family voucher" });
//   }
// };
...
//   // FIX: this type now exists after schema change
//   const where: Prisma.FamilyPaymentVoucherWhereInput = {};
...
//       {
//         parentUser: {
//           is: { fullName: { contains: String(q), mode: "insensitive" } },
//         },
//       },
//     ];
//   }
...
//     // type annotations to avoid implicit any
//     const items = rows.map((v) => {
//       const paid = Dsum(v.payments.map((p) => p.amountPaid as Prisma.Decimal));
//       const disc = Dsum(v.payments.map((p) => p.discount as Prisma.Decimal));
//       return {
//         id: v.id,
//         voucherNo: v.voucherNo,
//         method: v.method,
...
//     const totalPaid = Dsum(
//       voucher.payments.map((p) => p.amountPaid as Prisma.Decimal)
//     ).toNumber();
//     const totalDiscount = Dsum(
//       voucher.payments.map((p) => p.discount as Prisma.Decimal)
//     ).toNumber();
...
// export const listFamilyVouchersByParent = async (
//   req: Request,
//   res: Response
// ) => {
//   const parentUserId = Number(req.params.parentUserId);
...
//       ...v,
//       totalPaid: Dsum(
//         v.payments.map((p) => p.amountPaid as Prisma.Decimal)
//       ).toNumber(),
//       totalDiscount: Dsum(
//         v.payments.map((p) => p.discount as Prisma.Decimal)
//       ).toNumber(),
//       count: v.payments.length,
//     }));
...
// /** voidFamilyVoucher: update names + types */
// export const voidFamilyVoucher = async (req: Request, res: Response) => {
//   const voucherNo = String(req.params.voucherNo);
//   // @ts-ignore
//   const actor = req.user as { useId: number } | undefined;
...
//   try {
//     const result = await prisma.$transaction(async (tx: Tx) => {
//       const voucher = await (tx as any).familyPaymentVoucher.findUnique({
//         where: { voucherNo },
//         include: {
//           payments: { include: { allocations: true } },
//         },
...
//       for (const p of voucher.payments) {
//         if (p.allocations.length) {
//           // make Set<number> so feeId isn’t unknown
//           const feeIds: number[] = [
//             ...new Set(p.allocations.map((a) => a.studentFeeId)),
//           ];
...
//             });
//             const base = D(fee?.student_fee || 0);
//             const allocated = Dsum(
//               fee?.PaymentAllocation.map((x) => x.amount as Prisma.Decimal) ||
//                 []
//             );
//             const fully = base.gt(0) && allocated.gte(base);
//             await tx.studentFee.update({
...
//         const allocSum = Dsum(
//           p.allocations.map((a) => a.amount as Prisma.Decimal)
//         );
//         const remainder = D(p.amountPaid).minus(D(p.discount)).minus(allocSum);
//         if (remainder.gt(0)) {
//           const acct = await tx.studentAccount.findUnique({
...
//       const updated = await (tx as any).familyPaymentVoucher.update({
//         where: { id: voucher.id },
//         data: {
//           notes: `[VOIDED by ${actor.useId} at ${new Date().toISOString()}] ${
//             voucher.notes ?? ""
//           }`,
//         },
//       });
...
export const createFamilyVoucher = async (req: Request, res: Response) => {
  ...
  try {
    const voucher = await prisma.$transaction(async (tx: Tx) => {
      ...
      // Students must exist and belong to parent
      const studentIds = [
        ...new Set<number>(lines.map((l) => Number(l.studentId))),
      ];
      const kids = await tx.student.findMany({
        where: { id: { in: studentIds }, isdeleted: false },
        select: { id: true, parentUserId: true },
      ...
      for (const line of lines) {
        ...
        const payment = await tx.payment.create({
          data: {
            ...
            discount: disc,
            Description: notes || "Family voucher",
            // IMPORTANT: your current client expects this field name
            familyVoucherId: createdVoucher.id,
          },
        });
        ...
        const unpaidFees = await tx.studentFee.findMany({
          ...
          select: {
            id: true,
            student_fee: true,
            // IMPORTANT: your current client exposes this as PaymentAllocation (PascalCase)
            PaymentAllocation: { select: { amount: true } },
          },
        });
        ...
        for (const fee of unpaidFees) {
          ...
          const base = D(fee.student_fee || 0);
          const already = Dsum(
            fee.PaymentAllocation.map(
              (a: { amount: Prisma.Decimal | number | string }) => a.amount
            )
          );
          const due = base.minus(already);
          if (due.lte(0)) continue;
...
export const listFamilyVouchers = async (req: Request, res: Response) => {
  ...
  if (from || to) {
    where.createdAt = {};
    if (from) (where.createdAt as { gte?: Date }).gte = new Date(String(from));
    if (to) (where.createdAt as { lte?: Date }).lte = new Date(String(to));
  }
  ...
  try {
    const result = await prisma.$transaction(async (tx: Tx) => {
      ...
      for (const p of voucher.payments) {
        // reverse allocations
        if (p.allocations.length) {
          const feeIds: number[] = Array.from(
            new Set<number>(
              p.allocations.map((a: { studentFeeId: number }) => a.studentFeeId)
            )
          );
          ...
          for (const feeId of feeIds) {
            const fee = await tx.studentFee.findUnique({
              ...
              select: {
                student_fee: true,
                // IMPORTANT: relation list is PaymentAllocation
                PaymentAllocation: { select: { amount: true } },
              },
            });
            const base = D(fee?.student_fee ?? 0);
            const allocated = Dsum(
              (fee?.PaymentAllocation ?? []).map(
                (x: { amount: Prisma.Decimal | number | string }) => x.amount
              )
            );
            const fully = base.gt(0) && allocated.gte(base);
            await tx.studentFee.update({
...
/** ---------------------- OPTIONAL: Grouping endpoint kept for backward-compat ---------------------- */
/** month-key helpers (compare StudentFee.year/month to window) */
const monthKey = (y: number, m: number) => y * 12 + m; // m = 1..12
...
/**
 * GET /api/payments/family/groups
 * Supports older imports that expect listFamilyGroupsByParent
 */
export const listFamilyGroupsByParent = async (req: Request, res: Response) => {
  const {
    from,
  ...
  try {
    ...
    const payments = await prisma.payment.findMany({
      ...
      include: {
        ...
        // IMPORTANT: your client likely exposes the back link as familyVoucher
        familyVoucher: { select: { voucherNo: true } }, // may be null
        allocations: {
          select: {
            amount: true,
    ...
    const items = Array.from(byParent.values()).map((g) => {
      ...
      return {
        ...
        paymentsCount: g.paymentsCount,
        studentCount: students.length,
        voucherNos: g.voucherNos.sort(), // includes payment IDs when no voucher
        period: g.period,
        students,
      };
    });
